...
import com.example.batch.reader.DatabaseRecordReader;
...
public class CompareRecordsTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        List<CsvRecord> csvRecords = csvRecordReader.readAll();
        List<Record> dbRecords = databaseRecordReader.readAll();
    ...
    private Record findMatchingRecord(List<Record> dbRecords, CsvRecord csvRecord) throws ParseException {
        for (Record dbRecord : dbRecords) {
            if (Objects.equals(dbRecord.getCUST_ID_NO(), csvRecord.getCUST_ID_NO())
                    ...
                    && Objects.equals(convertDbToTimestamp(dbRecord.getADMIN_CRT_TMSTAMP().toString()), convertCsvToTimestamp(csvRecord.getADMIN_CRT_TMSTAMP()))
//                  && Objects.equals(dbRecord.getADMIN_CRT_TMSTAMP(), convertCsvToTimestamp(csvRecord.getADMIN_CRT_TMSTAMP()))
    ...
    private boolean compareAllFields(CsvRecord csvRecord, Record dbRecord) throws ParseException {
        return Objects.equals(convertDbToDate(dbRecord.getMTN_EFF_DT().toString()), convertCsvToDate(csvRecord.getMTN_EFF_DT()))
                && Objects.equals(convertDbToDate(dbRecord.getADMIN_EFF_DT().toString()), convertCsvToDate(csvRecord.getADMIN_EFF_DT()))
                && Objects.equals(csvRecord.getADMIN_CHG_AMT(), dbRecord.getADMIN_CHG_AMT())
                && Objects.equals(convertDbToDate(dbRecord.getBL_PER_FROM_DT().toString()), convertCsvToDate(csvRecord.getBL_PER_FROM_DT()))
                && Objects.equals(convertDbToDate(dbRecord.getBL_PER_TO_DT().toString()), convertCsvToDate(csvRecord.getBL_PER_TO_DT()))
                ...
                && Objects.equals(convertDbToTimestamp(dbRecord.getORIG_ADMIN_TMSTAMP().toString()), convertCsvToTimestamp(csvRecord.getORIG_ADMIN_TMSTAMP()))
                ...
                && Objects.equals(convertDbToTimestamp(dbRecord.getDB_TMSTAMP().toString()), convertCsvToTimestamp(csvRecord.getDB_TMSTAMP()))
                ...
                && Objects.equals(convertDbToTimestamp(dbRecord.getORIG_CREATE_TS().toString()), convertCsvToTimestamp(csvRecord.getORIG_CREATE_TS()))
